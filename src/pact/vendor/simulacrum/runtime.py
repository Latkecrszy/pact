...
Vendored from jmcentire/simulacrum origin/main@1dc4ccf3 under the MIT license.
Pact's only behavioral change is making the optional generalist branch truly
optional so specialist review requires only Anthropic.
...
CLASSIFIER_MODEL = os.environ.get("SIMULACRUM_CLASSIFIER_MODEL", "claude-sonnet-4-5")
SPECIALIST_MODEL = os.environ.get("SIMULACRUM_SPECIALIST_MODEL", "claude-sonnet-4-5")
...
def call_simulacrum(question, history, temperature=0.7) -> dict:
    try:
        ...
        sys.exit(
            ...
            "Install Pact review support with: pip install 'pact-agents[review]'"
        )
    ...
    if phase == "GENERALIST" and GENERALIST_MODEL:
        try:
            import openai
        except ImportError as e:
            sys.exit(
                f"Missing dependency: {e}. "
                "Install the optional generalist branch with: "
                "pip install 'pact-agents[openai]'"
            )
        ...
        if phase == "GENERALIST":
            phase_reason = (
                f"{phase_reason} Generalist branch is disabled; routed to specialist."
            ).strip()
            phase = "SPECIALIST"
